// import React, { useState, useEffect } from "react";
// import { Link } from "react-router-dom";
// import "./Products.css";
// import helmetImage from "../assets/helmet.jpg";
// import chatbotImage from "../assets/chatbot.jpg";
// import operationAdvImage from "../assets/operation-adv.jpg";
// import netmanImage from "../assets/netman.jpg";

// const productsData = [
//   {
//     id: 1,
//     title: "SMART HELMET",
//     image: helmetImage,
//     points: [
//       "Smart Helmet is an IoT-powered safety device.",
//       "Designed for Industrial environments",
//       "Enhances safety through real-time monitoring",
//       "Improves operational safety & worker protection."
//     ]
//   },
//   {
//     id: 2,
//     title: "CHATBOT",
//     image: chatbotImage,
//     points: [
//       "Enterprise-grade AI chatbot for automation",
//       "Enables intelligent, human-like conversations",
//       "Provides real-time support and accurate replies",
//       "Understands context and user intent."
//     ]
//   },
//   {
//     id: 3,
//     title: "OPERATION ADVISORS",
//     image: operationAdvImage,
//     points: [
//       "Operation Advisory & Managed Services",
//       "Seamless IT & network management",
//       "Performance optimization & less downtime",
//       "Proactive monitoring & analytics"
//     ]
//   },
//   {
//     id: 4,
//     title: "COMMEDIA NETWORK MANAGEMENT SYSTEM",
//     image: netmanImage,
//     points: [
//       "Real-time Network Monitoring",
//       "End-to-End Visibility",
//       "Proactive Fault Detection",
//       "Centralized Dashboard Control"
//     ]
//   }
// ];

// const Products = () => {
//   const [currentIndex, setCurrentIndex] = useState(0);
//   const delay = 4000;

//   // ✅ Scroll to top on load
//   useEffect(() => {
//     window.scrollTo(0, 0);
//   }, []);

//   // ✅ Slideshow auto-change
//   useEffect(() => {
//     const interval = setInterval(() => {
//       setCurrentIndex((prevIndex) => (prevIndex + 1) % productsData.length);
//     }, delay);
//     return () => clearInterval(interval);
//   }, [currentIndex]);

//   return (
//     <div className="products-container">
//       <h2 className="products-title">PRODUCTS</h2>
//       <div className="products-content products-fade-in">
//         <div className="products-image">
//           <img
//             src={productsData[currentIndex].image}
//             alt={productsData[currentIndex].title}
//             className="product-image"
//           />
//         </div>
//         <div className="products-text">
//           <h3>{productsData[currentIndex].title}</h3>
//           <ul>
//             {productsData[currentIndex].points.map((point, index) => (
//               <li key={index}>{point}</li>
//             ))}
//           </ul>
//           <Link to="/products" className="products-read-more">Read more</Link>
//         </div>
//       </div>
//     </div>
//   );
// };

// export default Products;

import React, { useState, useEffect } from 'react';
import { Link } from 'react-router-dom';
import "./Products.css";

// Import your images
import helmetImage from '../assets/helmet.jpg';
import chatbotImage from '../assets/chatbot.jpg';
import operationAdvImage from '../assets/operation-adv.jpg';
import netmanImage from '../assets/netman.jpg';

const productsData = [
  {
    id: 1,
    title: "SMART HELMET",
    image: helmetImage,
    points: [
      "IoT-powered safety device for industrial environments",
      "Real-time monitoring system",
      "Enhanced operational safety protocols",
      "Worker protection enhancement system"
    ]
  },
  {
    id: 2,
    title: "CHATBOT",
    image: chatbotImage,
    points: [
      "Enterprise-grade AI automation",
      "Natural language processing capabilities",
      "24/7 real-time support system",
      "Context-aware interaction engine"
    ]
  },
  {
    id: 3,
    title: "OPERATION ADVISORS",
    image: operationAdvImage,
    points: [
      "Full-spectrum IT management",
      "Network performance optimization",
      "Proactive monitoring solutions",
      "Downtime reduction strategies"
    ]
  },
  {
    id: 4,
    title: "NETWORK MANAGEMENT SYSTEM",
    image: netmanImage,
    points: [
      "Real-time network analytics",
      "End-to-end visibility dashboard",
      "Automated fault detection",
      "Centralized control interface"
    ]
  }
];

const Products = () => {
  const [currentIndex, setCurrentIndex] = useState(0);

  useEffect(() => {
    const timer = setInterval(() => {
      setCurrentIndex(prev => (prev + 1) % productsData.length);
    }, 4000);
    return () => clearInterval(timer);
  }, []);

  const currentProduct = productsData[currentIndex];

  return (
    <section className="products-section">
      <h2 className="products-title">PRODUCTS</h2>
      
      <div className="products-slider">
        <div className="product-slide">
          <div className="product-image-container">
            <img 
              src={currentProduct.image} 
              alt={currentProduct.title} 
            />
          </div>
          
          <div className="product-content">
            <h3>{currentProduct.title}</h3>
            <hr />
            <ul>
              {currentProduct.points.map((point, index) => (
                <li key={index}>{point}</li>
              ))}
            </ul>
            <Link to="/products" className="product-read-more">
              Read More
            </Link>
          </div>
        </div>
      </div>
    </section>
  );
};

export default Products;


/// v2 responsive working version //


// import React, { useState, useEffect } from "react";
// import { Link } from "react-router-dom";
// import "./Products.css";

// import helmetImage from "../assets/helmet.jpg";
// import chatbotImage from "../assets/chatbot.jpg";
// import operationAdvImage from "../assets/operation-adv.jpg";
// import netmanImage from "../assets/netman.jpg";

// const productsData = [
//   {
//     id: 1,
//     title: "SMART HELMET",
//     image: helmetImage,
//     points: [
//       "Smart Helmet is an IoT-powered safety device for industrial use",
//       "Designed for environments like manufacturing, mining, oil rigs, and construction",
//       "Enhances workplace safety through real-time monitoring",
//       "Ideal for improving operational safety and worker protection."
//     ],
//   },
//   {
//     id: 2,
//     title: "CHATBOT",
//     image: chatbotImage,
//     points: [
//       "Enterprise-grade AI chatbot for automation",
//       "Enables intelligent, human-like conversations",
//       "Provides real-time support and accurate replies",
//       "Understands context and user intent."
//     ],
//   },
//   {
//     id: 3,
//     title: "OPERATION ADVISORS",
//     image: operationAdvImage,
//     points: [
//       "Operation Advisory & Managed Services",
//       "Seamless IT & network management",
//       "Performance optimization & less downtime",
//       "Proactive monitoring & analytics"
//     ],
//   },
//   {
//     id: 4,
//     title: "COMMEDIA NETWORK MANAGEMENT SYSTEM",
//     image: netmanImage,
//     points: [
//       "Real-time Network Monitoring",
//       "End-to-End Visibility",
//       "Proactive Fault Detection",
//       "Centralized Dashboard Control"
//     ],
//   }
// ];

// export default function Products() {
//   const [current, setCurrent] = useState(0);

//   useEffect(() => {
//     const timer = setInterval(() => {
//       setCurrent(idx => (idx + 1) % productsData.length);
//     }, 4000);
//     return () => clearInterval(timer);
//   }, []);

//   const { title, image, points } = productsData[current];

//   return (
//     <section className="products-section">
//       <h2 className="products-title">PRODUCTS</h2>
//       <div className="products-slider">
//         <div className="product-slide">
//           <div className="product-image">
//             <img src={image} alt={title} />
//           </div>
//           <div className="product-content">
//             <h3>{title}</h3>
//             <hr />
//             <ul>
//               {points.map((p, i) => (
//                 <li key={i}>{p}</li>
//               ))}
//             </ul>
//             <Link to="/products" className="product-button">
//               Read more
//             </Link>
//           </div>
//         </div>
//       </div>
//     </section>
//   );
// }
